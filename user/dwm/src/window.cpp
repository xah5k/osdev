...
uint64_t Window::DrawDecoration() {
    if ((FullWindowRedraw || DecorationRedraw) && (this->Options == DWMPCK_CRWIN_DEFAULT || this->Options == DWMPCK_CRWIN_WINDOWED)) {
        ...
        OsDrawTextAtFb(this->dwm->GetFrontBuffer(), this->Name.c_str(), this->Pos.x, this->Pos.y, DWM_WINDOW_TITLEBAR_TEXTCOLOUR, this->IsActive ? DWM_WINDOW_TITLEBAR_COLOUR_ACTIVE : DWM_WINDOW_TITLEBAR_COLOUR_INACTIVE);
