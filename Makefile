...
run-x86_64-uefi:
	qemu-system-x86_64 -net none -net none -netdev tap,id=net0,ifname=tap0,script=no,downscript=no -device rtl8139,netdev=net0,mac=52:54:00:12:34:56 -net user -m 128M -M q35 -drive file=osdev.img,format=raw $(QARG) -serial stdio -drive if=pflash,unit=0,format=raw,file=edk2-ovmf/ovmf-code-$(ARCH).fd,readonly=on
